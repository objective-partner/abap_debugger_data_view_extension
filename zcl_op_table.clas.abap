  ...
  METHOD add_itab.
    ...
    LOOP AT i_table ASSIGNING FIELD-SYMBOL(<table_line>).
      ...
      LOOP AT field_catalog INTO DATA(field_info).
        ASSIGN COMPONENT field_info-fieldname OF STRUCTURE <table_line> TO FIELD-SYMBOL(<field>).
        CHECK <field> IS ASSIGNED AND
              <field> IS NOT INITIAL AND
              field_info-seltext NE |INDEX|.

        DATA(varvalue) = COND tpda_var_value( WHEN field_info-datatype = |TTYP| THEN me->add_itab( i_current_context  = space
                                                                                                   i_table            = <field>
                                                                                                   i_dont_add_a_point = abap_true )
                                                 WHEN field_info-datatype = |STRU| THEN structure->add_structure( i_structure       = <field>
