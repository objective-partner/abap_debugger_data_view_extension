...
CLASS zcl_op_value_pretty_printer DEFINITION
  ...
  PRIVATE SECTION.
    ...
    CONSTANTS: c_newline              TYPE abap_char1 VALUE cl_abap_char_utilities=>cr_lf,
               c_regex_lhs            TYPE string     VALUE `[^=\s]+(?=\s*=)` ##NO_TEXT,
    ...
    METHODS: get_spaces RETURNING VALUE(r_spaces_string) TYPE string,
      get_offset_in_current_line IMPORTING i_content       TYPE string
                                 RETURNING VALUE(r_offset) TYPE i,
      get_offset_for_apostroph IMPORTING i_content       TYPE string
                               RETURNING VALUE(r_offset) TYPE i,
      get_offset_for_closing_bracket IMPORTING i_content       TYPE string
                                     RETURNING VALUE(r_offset) TYPE i,
      clean_up_steering_tab    IMPORTING i_last_char TYPE c,
      ...
      non_terminal IMPORTING i_sub_terminal TYPE enum_sub_terminal
                             i_regex        TYPE csequence
                   RAISING   lcx_no_match,

      add_text_literal   RAISING lcx_no_match,
      add_string_literal RAISING lcx_no_match,
      set_offset_for_regex IMPORTING i_regex TYPE csequence
                           RAISING   lcx_no_match,
      add_to_trace,
      set_sync_point IMPORTING i_terminal                        TYPE enum_terminal
                               i_sub_terminal                    TYPE enum_sub_terminal DEFAULT c_sub_terminal-na
                     RETURNING VALUE(r_current_sync_point_index) TYPE i,
      reset_to_sync_point     IMPORTING i_sync_point_index TYPE i,
      cancel_sync_point       IMPORTING i_sync_point_index TYPE i,
...
CLASS zcl_op_value_pretty_printer IMPLEMENTATION.
  METHOD get_spaces.
    DATA(spaces_no) = steering[ lines( me->steering ) ]-offset_no.
    r_spaces_string = repeat( val = ` `
                              occ = spaces_no ).
  ENDMETHOD.
  ...
  METHOD get_offset_for_closing_bracket.
    " TODO: parameter I_CONTENT is never used (ABAP cleaner)

    " find offset of corresponding opening bracket
    " therefore we need to loop backwards in steering table
  ...
  METHOD clean_up_steering_tab.
    CHECK i_last_char = |)|.
    " a formatting block is complete
    " we need to clean up our steering data and remove infos about this complete formatting block
    DATA(x_lines) = lines( me->steering ).
...
*    table : 'VALUE #(' ( '( )' | ( '(' RHS2 ')' )+ ) ')'
  ...
  METHOD lhs.
    TRY.
        ...
        set_offset_for_regex_pcre( c_regex_lhs ).
        add_to_trace( ).
      CLEANUP.
        reset_to_sync_point( current_sync_point_index ).
    ENDTRY.
  ...
  METHOD non_terminal.
    TRY.
        find_spaces( ).
        DATA(current_sync_point_index) = set_sync_point( i_terminal     = c_terminal-non_terminal
                                                         i_sub_terminal = i_sub_terminal ).
  ...
  METHOD find_spaces.
    ...
    IF sy-subrc = 0.
      APPEND VALUE ty_trace( sync_point        = 0
                             parent_sync_point = 0
  ...
  METHOD set_offset_for_regex.
    IF me->offset >= strlen( me->text ).
      RAISE EXCEPTION NEW lcx_no_match( ).
    ENDIF.
    DATA(regex) = |^{ i_regex }|.
    FIND FIRST OCCURRENCE OF REGEX regex IN me->text+me->offset MATCH LENGTH DATA(length).
    IF sy-subrc = 0.
      offset += length.
    ELSE.
      RAISE EXCEPTION NEW lcx_no_match( ).
  ...
  METHOD zif_op_value_pretty_printer~format.
    ...
    TRY.
      ...
      lcx_no_match INTO DATA(lx_error).
        DATA(error_text) = lx_error->get_longtext( ).
        zcl_op_debugger_integration=>debug_debugger_if_needed( ).
    ENDTRY.

    DATA(indent) = 0.
    DATA(indent_step) = 8.
    DATA(indent_spaces) = repeat( val = ` `
                                  occ = indent ).
    ...
    LOOP AT me->trace_tab REFERENCE INTO DATA(trace_line).
      ...
      CASE trace_line->terminal.
        WHEN c_terminal-symbolname
          ...
          r_formated_content = r_formated_content && substring( val = i_unformated_value_content
                                                                off = trace_line->offset
                                                                len = trace_line->length ).
      ENDCASE.
      ...
      ELSEIF    trace_line->sub_terminal = c_sub_terminal-value_operator
        ...
        indent += indent_step.
        indent_spaces = indent_spaces && repeat( val = ` `
                                                 occ = indent_step ).
        r_formated_content = |{ r_formated_content }\n{ indent_spaces }|.
  ...
  METHOD add_to_trace.
    ...
    APPEND VALUE ty_trace( LET length            = offset - <sync_point>-offset IN
    ...
    IF 0 = 1.
      RAISE EXCEPTION NEW lcx_parser_interrupt( ).
    ENDIF.
  ENDMETHOD.

  METHOD set_sync_point.
    APPEND VALUE ty_sync_point( terminal     = i_terminal
  ...
  METHOD reset_to_sync_point.
    ...
    offset = sync_points[ i_sync_point_index ]-offset.
  ...
  METHOD add_value_open_operator.
    non_terminal( i_sub_terminal = c_sub_terminal-value_operator
                  i_regex        = `VALUE #\(` ).
  ENDMETHOD.

  METHOD add_parenthesis_close.
    non_terminal( i_sub_terminal = c_sub_terminal-parenthesis_close
                  i_regex        = `\)` ).
  ENDMETHOD.

  METHOD add_assign_operator.
    non_terminal( i_sub_terminal = c_sub_terminal-assign_operator
                  i_regex        = '=' ).
  ENDMETHOD.

  METHOD add_end_of_statement.
    non_terminal( i_sub_terminal = c_sub_terminal-end_of_statement
                  i_regex        = '\.' ).
  ENDMETHOD.

  METHOD add_empty_line.
    non_terminal( i_sub_terminal = c_sub_terminal-empty_line
                  i_regex        = '\( \)' ).
  ENDMETHOD.

  METHOD add_parenthesis_open.
    non_terminal( i_sub_terminal = c_sub_terminal-parenthesis_open
                  i_regex        = '\(' ).
  ENDMETHOD.

  METHOD set_offset_for_regex_pcre.
 IF me->offset >= strlen( me->text ).
      RAISE EXCEPTION NEW lcx_no_match( ).
    ENDIF.
    DATA(regex) = |^{ i_regex }|.
    FIND FIRST OCCURRENCE OF PCRE regex in me->text+me->offset MATCH LENGTH DATA(length).
    IF sy-subrc = 0.
      offset += length.
    ELSE.
      RAISE EXCEPTION NEW lcx_no_match( ).
