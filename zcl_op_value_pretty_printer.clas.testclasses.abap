...
CLASS ltcl_pritty_printer_should DEFINITION DEFERRED.
CLASS ltcl_pretty_printer_performanc DEFINITION DEFERRED.
CLASS zcl_op_value_pretty_printer DEFINITION LOCAL FRIENDS ltcl_pritty_printer_should ltcl_pretty_printer_performanc.
CLASS ltcl_pritty_printer_should DEFINITION FINAL
  ...
  PRIVATE SECTION.
    ...
    METHODS: setup,
             ...
             format_field_symbol_table FOR TESTING.
ENDCLASS.


CLASS ltcl_pretty_printer_performanc DEFINITION FINAL
  FOR TESTING RISK LEVEL HARMLESS DURATION MEDIUM.

  PRIVATE SECTION.
    METHODS string_literal FOR TESTING RAISING cx_static_check.
ENDCLASS.


CLASS ltcl_pritty_printer_should IMPLEMENTATION.
  METHOD setup.
  ...
  METHOD add_indent.
    ...
    " THEN
    DATA(expected) = concat_lines_of( table = VALUE string_table( ( |IS_NESTED_STRUCTURE = VALUE #(| )
                                                                  ( |         COL1 = '1'| )
                                                                  ( |         COL2 = VALUE #(| )
                                                                  ( |                 COL1 = '1'| )
                                                                  ( |                 COL2 = '2'| )
  ...
  METHOD table_of_integers.
    ...
    " THEN
    DATA(expected) = concat_lines_of( table = VALUE string_table( ( |TABLE_OF_INTEGERS = VALUE #(| )
                                                                  ( |         (| )
                                                                  ( |                 1 )| )
                                                                  ( |         (| )
                                                                  ( |                 2 )| )
  ...
  METHOD table_in_structure.
    ...
    " THEN
    DATA(expected) = concat_lines_of( table = VALUE string_table( ( |TABLE_IN_STRUCTURE = VALUE #(| )
                                                                  ( |         TABLE = VALUE #(| )
                                                                  ( |                 (| )
                                                                  ( |                         COL1 = '1'| )
                                                                  ( |                         COL2 = '2'| )
  ...
  METHOD format_field_symbol_table.
    " GIVEN
    DATA(input) =    |<FLIGHTS> = VALUE #( ( MANDT = '100' CARRID = 'AA' CONNID = '0017' FLDATE = '20141217'| &&
                     |PRICE = '422.94 ' CURRENCY = 'USD' PLANETYPE = '747-400' SEATSMAX = 385 SEATSOCC = 372| &&
                     |PAYMENTSUM = '192437.84 ' SEATSMAX_B = 31 SEATSOCC_B = 28 SEATSMAX_F = 21 SEATSOCC_F =| &&
                     |21 ) ( MANDT = '100' CARRID = 'AA' CONNID = '0017' FLDATE = '20150225' PRICE = '422.94 '| &&
                     | CURRENCY = 'USD' PLANETYPE = '747-400' SEATSMAX =385 SEATSOCC = 369 PAYMENTSUM =| &&
                     | '191490.49 ' SEATSMAX_B = 31 SEATSOCC_B = 30 SEATSMAX_F = 21 SEATSOCC_F = 20 ) (| &&
                     | MANDT = '100' CARRID = 'AA' CONNID = '0017' FLDATE = '20150506' PRICE = '422.94 '| &&
                     | CURRENCY = 'USD' PLANETYPE = '747-400' SEATSMAX = 385 SEATSOCC = 374 PAYMENTSUM =| &&
                     | '192991.93 ' SEATSMAX_B = 31 SEATSOCC_B = 30 SEATSMAX_F = 21 SEATSOCC_F = 19 ) ).|.

    " WHEN
    DATA(formated_string) = cut->format( input ).

    " THEN
    cl_abap_unit_assert=>assert_not_initial( act = formated_string ).
  ENDMETHOD.
ENDCLASS.
...
CLASS ltcl_pretty_printer_performanc IMPLEMENTATION.
  METHOD string_literal.
    ...
    input = |MAIN_RECORD_TABLE = VALUE #( |
        && repeat(
            occ = occ
            val = |( DURATION = '62 ' STATEMENT_WITH_VALUES = `OTHER (SQL)` RECORD_NUMBERS = VALUE #( ( 1 ) ( 59 ) ( 94 ) ( 837 ) ( 2763 ) ( 3023 ) ( 3047 ) ( 3707 ) ( 3783 ) ( 3798 ) (|
